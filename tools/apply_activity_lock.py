...
# ---------------------------------------------------------------------------
# Config: persistent settings stored in the existing QSettings backend.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Physical buttons: block navigation while keeping volume/mute/play/power.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Touch slider: expose lock state and make normal slider consumers ignore it.
# ---------------------------------------------------------------------------
...
Item {
    ...
    Connections {
        ...
        function onActivityLockEnabledChanged() {
            if (!Config.activityLockEnabled) lock.setLocked(false); else lock.resetInactivityTimer();
        }
    ...
    Rectangle {
        ...
        Column {
            ...
            spacing: 24
            Text { width: parent.width; horizontalAlignment: Text.AlignHCenter; color: "white"; text: "🔒"; font.pixelSize: 70 }
            Text { width: parent.width; horizontalAlignment: Text.AlignHCenter; color: "white"; text: qsTr("Remote locked"); font.pixelSize: 34 }
            Text {
                width: parent.width; horizontalAlignment: Text.AlignHCenter; wrapMode: Text.WordWrap; color: "#dddddd"
            ...
            Text { width: parent.width; horizontalAlignment: Text.AlignHCenter; color: "#aaaaaa"; text: qsTr("The current activity and page remain selected."); font.pixelSize: 18 }
        }
    }
}
''')
...
Settings.Page {
    ...
    Flickable {
        ...
        width: parent.width; contentWidth: width; contentHeight: content.height + 30; clip: true
        ColumnLayout {
            id: content; width: parent.width; spacing: 20
            ColumnLayout {
                Layout.leftMargin: 10; Layout.rightMargin: 10; spacing: 10
                RowLayout {
                    ...
                    Components.Switch { checked: Config.activityLockEnabled; icon: "uc:check"; trigger: function() { Config.activityLockEnabled = !Config.activityLockEnabled; } }
                }
                Text { Layout.fillWidth: true; color: colors.light; text: qsTr("Locks the current activity and page against accidental navigation input."); font: fonts.secondaryFont(24); wrapMode: Text.WordWrap }
            }
            ...
            ColumnLayout {
                ...
                Components.Slider {
                    Layout.fillWidth: true; from: 0; to: 30; stepSize: 1; showLiveValue: false
                    value: Math.round(Config.activityLockInactivityTimeoutSec / 60)
                    lowValueText: qsTr("Off"); highValueText: qsTr("30 min")
                    onMoved: Config.activityLockInactivityTimeoutSec = Math.round(value) * 60
                }
                Text { Layout.fillWidth: true; color: colors.light; text: qsTr("Independent of the remote standby timer."); font: fonts.secondaryFont(24); wrapMode: Text.WordWrap }
            }
            Rectangle { Layout.fillWidth: true; Layout.leftMargin: 10; Layout.rightMargin: 10; height: 2; color: colors.medium }
            RowLayout {
                ...
                Components.Switch { checked: Config.activityLockOnWake; icon: "uc:check"; trigger: function() { Config.activityLockOnWake = !Config.activityLockOnWake; } }
            }
            Rectangle { Layout.fillWidth: true; Layout.leftMargin: 10; Layout.rightMargin: 10; height: 2; color: colors.medium }
            ColumnLayout {
                Layout.leftMargin: 10; Layout.rightMargin: 10; spacing: 10
                Text { color: colors.offwhite; text: qsTr("Unlock methods"); font: fonts.primaryFont(30) }
                RowLayout { Text { Layout.fillWidth: true; color: colors.offwhite; text: qsTr("Touchscreen swipe"); font: fonts.primaryFont(26) }; Components.Switch { checked: Config.activityLockTouchUnlock; icon: "uc:check"; trigger: function() { if (Config.activityLockSliderUnlock) Config.activityLockTouchUnlock = !Config.activityLockTouchUnlock; } } }
                RowLayout { Text { Layout.fillWidth: true; color: colors.offwhite; text: qsTr("Touch slider"); font: fonts.primaryFont(26) }; Components.Switch { checked: Config.activityLockSliderUnlock; icon: "uc:check"; trigger: function() { if (Config.activityLockTouchUnlock) Config.activityLockSliderUnlock = !Config.activityLockSliderUnlock; } } }
                Text { Layout.fillWidth: true; color: colors.light; text: qsTr("At least one unlock method is always required."); font: fonts.secondaryFont(22); wrapMode: Text.WordWrap }
            }
        }
    }
}
...
# ---------------------------------------------------------------------------
# Main UI and resource collection.
# ---------------------------------------------------------------------------
main = read("src/qml/main.qml")
if 'Components.ActivityLock {' not in main:
    marker = "\n}\n"
    pos = main.rfind(marker)
    if pos < 0: raise RuntimeError("main.qml closing brace not found")
    main = main[:pos] + "\n    Components.ActivityLock { id: activityLock; anchors.centerIn: parent }\n" + main[pos:]
...
if 'components/ActivityLock.qml' not in qrc:
    ...
    if anchor not in qrc: raise RuntimeError("main.qrc component anchor not found")
...
if 'settings/settings/ActivityLock.qml' not in qrc:
    ...
    if anchor not in qrc: raise RuntimeError("main.qrc settings anchor not found")
